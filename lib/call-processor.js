  ...
  start(srf, rtpEngine, registrar) {
    ...
    srf.invite(async(req, res) => {
      ...
      if (registrar.hasUser(user)) {
        ...
        this.logger.debug(`inbound call with details: ${JSON.stringify(details)}`) ;
      }
      ...
      try {
        this.logger.debug({offer: optsOffer}, 'about to make initial offer to rtpengine');
  ...
  async _handleAck(dlg, answer, offer, ack, offerSdp, rtpEngineOpts, req) {
    this.logger.info('Received ACK with late offer: ', req.body);
      ...
      try {
        ...
        if (JSON.stringify(answerMedia).includes('ICE\":\"remove')) {
          ackResponse.sdp = utils.removeWebrtcAttributes(ackResponse.sdp);
        }
